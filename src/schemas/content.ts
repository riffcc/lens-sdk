...
const resolvePathFromProperties = (
    ...
    if ('parent' in properties && properties.parent) {
        ...
        return ('path' in properties && Array.isArray((properties as { path: CanvasReference[] }).path))
            ? (properties as { path: CanvasReference[] }).path
            : [];
    }
};
...
export const LOWEST_QUALITY = 0 as const;
export const MEDIUM_QUALITY = 1431655765 as const; // 33% of max u32
export const HIGH_QUALITY = 2863311530 as const; // max u32
export const HIGHEST_QUALITY = 4294967295 as const; // max u32
...
export class Canvas extends Program<CanvasArgs> {
    ...
    private async maybeSave() {
        // update address
        const addressBefore = this.address;
    ...
    async open(args?: CanvasArgs): Promise<void> {
        ...
        if (!this.isOrigin) {
            ...
            if (this.debug) {
                ...
                console.time(this.getValueWithContext('openReplies'));
            }
    ...
    private async reIndex(
        ...
        if (!context) {
            ...
            const indexedCanvas = await parent.replies.index.get(canvas.id, {
                resolve: false,
                local: true,
                remote: {
                    strategy: 'fallback',
    ...
    async afterOpen(): Promise<void> {
        ...
        if (this._repliesChangeListener) {
    ...
    async load() {
        ...
        if (this.node) {
            await Promise.resolve(this.node);
        } else {
    ...
    get replies(): Documents<Canvas, IndexableCanvas> {
        const root: Canvas = this.origin ?? this;
        return root._replies;
    }

    get visualizations(): Documents<Visualization, IndexedVisualization> {
        const root: Canvas = this.origin ?? this;
    ...
    get elements(): Documents<Element, IndexableElement> {
        const root: Canvas = this.origin ?? this;
    ...
    async loadContext(options?: {
        ...
        if ((this as unknown as WithContext<Canvas>).__context && !options?.reload) {
            return (this as unknown as WithContext<Canvas>).__context;
        }
        ...
        return ((this as unknown as WithContext<Canvas>).__context = withContext);
    }

    get loadedContext(): boolean {
        return (this as unknown as WithContext<Canvas>).__context != null;
    }

    get context(): Context | null {
        if ((this as unknown as WithContext<Canvas>).__context) {
            return (this as unknown as WithContext<Canvas>).__context;
        }
        return null;
    }
    ...
    async openWithSameSettings<T extends Canvas | WithContext<Canvas>>(
        ...
        const context = (other as unknown as WithContext<Canvas>).__context;
        const replies = this._replies.closed ? this.replies : this._replies;
        const replicating = await replies.log.isReplicating();
        const minReplicas = replies.log.replicas.min.getValue(replies.log);
