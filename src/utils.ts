import { Ed25519PublicKey, fromHexString, Secp256k1PublicKey, type PublicSignKey } from '@peerbit/crypto';
...
export function publicSignKeyFromString(keyString: string): PublicSignKey {
    ...
    } else if (typeIdentifier === 'sepc256k1') { // Matches the 'sepc256k1' prefix in toString()
        if (keyBytes.length !== 33) {
            throw new Error(`Invalid Secp256k1 public key length. Expected 33, got ${keyBytes.length}`);
        }
